...
## [2026-07-14 11:25] src-eurocommemo — Batch Sendcloud shipping-options on "ready to ship" order list

**Target**: src-eurocommemo @ main
**Status**: SUCCESS
**Files affected**:
- `src/Service/Sendcloud/SendcloudApiClient.php` (modified) — added imports `GuzzleHttp\Pool` and `GuzzleHttp\Psr7\Request`, constant `SHIPPING_OPTIONS_CONCURRENCY = 10`, and new public method `getShippingOptionsBatch(array $shipments): array`. Takes shipment descriptors keyed by a caller-defined dedup key, fires all `POST /api/v3/shipping-options` concurrently via a Guzzle `Pool` (same body shape and `calculate_quotes: true` as `getShippingOptions()`), and returns `array<key, SendcloudShippingOptionDTO[]>`. Reuses the Basic-auth logic from `request()`; a non-2xx/rejected request yields `[]` for its key.
- `src/Controller/Admin/OrderCrudController.php` (modified) — new route `sendcloudShippingOptionsBatch(Request)` (`POST /admin/order/sendcloud/shipping-options/batch`, name `app_admin_order_sendcloud_shipping_options_batch`), inserted before the per-order `sendcloudShippingOptions()`. Reads `ids[]`, loads orders via `findBy(['id' => ...])`, deduplicates Sendcloud calls by exact signature `md5(country|postal|city|weight(3dp))`, calls `getShippingOptionsBatch()`, and returns `{orderId: [{code,label,carrierCode,requiresServicePoint}]}` (same unit shape as the per-order route). Returns HTTP 502 `{error}` on `ExternalSendcloudApiException`.
- `templates/admin/order/order_sendcloud_action.html.twig` (modified, l.6) — added `data-batch-url="{{ path('app_admin_order_sendcloud_shipping_options_batch') }}"` on `.sendcloud-shipping-block`. The now-unused `data-url` on the shipping `<select>` was left in place (inoffensive).
- `assets/back/js/back.js` (modified, Sendcloud block) — replaced the per-row `fetch(shippingSelect.dataset.url)` loop (N HTTP calls) with a single `fetch(batchUrl, {method:'POST', body: ids[]})`. Each row registers a `populate(options)` closure (identical option-rendering markup as before); the batch response `{orderId: [...]}` is redistributed to each row. The global overlay is now removed in the fetch's `.finally()` (replaces `Promise.allSettled`). Service-point loading, the `change` listener and the "Enregistrer" apply handler are unchanged.
- `public/build/*` (regenerated) — Encore production build (62 files); compiled `app-admin.ad6a804a.js` contains the `batchUrl`/`ids[]` markers.

**Notes**: Implements `plans/2026-07-14_sendcloud-batch-shipping-options.md`. Answers the user's question: a single Sendcloud call covering all orders is impossible (the `/shipping-options` endpoint is per-destination + per-weight), so the optimization collapses the browser's N requests into 1 and deduplicates + parallelizes the Sendcloud calls server-side (exact-signature dedup, zero behaviour change). Verified in `php-fpm-per83` via `scripts/repo_exec.py`: `php -l` clean on both PHP files, `lint:container` OK, both routes registered in `debug:router` with no collision (`/sendcloud/shipping-options/batch` vs `/{id}/sendcloud/shipping-options`), and `GuzzleHttp\Pool` + `GuzzleHttp\Psr7\Request` resolve from vendor. Frontend built in the `encore` container (node:18-alpine, project at `/eurocommemo`) via `docker compose exec` — NOT `repo_exec.py` (php-fpm-per83 has no Node). Graphify graph refreshed (4 files re-extracted, re-clustered). **Not verified**: no live browser run against the admin page (no end-to-end confirmation that the batch endpoint returns the same options as the per-order route against real Sendcloud data). No commit made (awaiting explicit user instruction).

## [2026-07-14 11:40] src-eurocommemo — Differentiate Sendcloud link per order-list view

**Target**: src-eurocommemo @ main
**Status**: SUCCESS
**Files affected**:
- `templates/admin/order/order_ebay_shipping_service.html.twig` (modified, link block) — made the Sendcloud link conditional on the `delivery` query parameter. On the "Prêtes à être expédiées" view (`app.request.query.get('delivery') == 1`) it now renders the restored **"Voir sur Sendcloud"** back-office button (`https://app.sendcloud.com/v2/shipping/list/orders?search=<orderIdEbay>`, shown when `sendcloudOrderId` is set/`!= '0'` and `orderIdEbay` present — logic from commit `c34374e`). On the "Toutes les commandes" view (no `delivery` param) it keeps the **"Suivre le colis"** carrier tracking link (`entity.instance.sendcloudTrackingUrl`, with `rel="noopener noreferrer"`). The `shippingService` display block (l.1-15) is unchanged.

**Notes**: Implements the approved plan `~/.claude/plans/dans-un-pr-c-dent-commit-bubbly-cerf.md`. Reverses the blanket replacement made in `fe6b73d` only for the ready-to-ship view. Template-only change — no PHP/entity/JS. Both getters already exist (`Order::getSendcloudOrderId()` l.458, `getSendcloudTrackingUrl()` l.524). `lint:twig` OK via `scripts/repo_exec.py`. **Not verified**: no live browser run against the admin page. No commit made (awaiting explicit user instruction).

## [2026-07-15 00:48] src-eurocommemo — Sort Sendcloud shipping options alphabetically

**Target**: src-eurocommemo @ main
**Status**: SUCCESS
**Files affected**:
- `src/Service/Sendcloud/SendcloudApiClient.php` (modified) — added private static helper `sortOptionsByLabel(array $options): array` that sorts `SendcloudShippingOptionDTO[]` by their displayed `getLabel()` using a locale-aware `\Collator('fr_FR')` (`compare(...) ?: 0`), so accented French carrier/method names order correctly and case-insensitively. Applied in both `getShippingOptions()` (single-order, wraps the returned `array_map`) and the `fulfilled` callback of `getShippingOptionsBatch()` (per-key result, called as `self::` since the closure is static). Fixes the previously implicit API-response ordering, giving a consistent A→Z order in every Sendcloud `<select>` on the "Prêtes à être expédiées" page.

**Notes**: Implements the approved plan `~/.claude/plans/sur-le-selecteur-du-expressive-meteor.md`. Backend-only single-file change — no controller/JS/Twig edit needed (the `OrderCrudController` payload `array_map` and `back.js` `populate()` both preserve the received order, so they inherit the sort). Verified in `php-fpm-per83` via `scripts/repo_exec.py`: `ext-intl` present (`INTL_YES`), `php -l` clean, and a standalone Collator sort sanity-check produced the expected accent-aware order (`Chronopost | colissimo | Éco | Étranger | Mondial Relay`). The JS placeholder option "— Choisir une méthode —" stays first (prepended client-side, outside the sorted list). **Not verified**: no live browser run against the admin page. No commit made (awaiting explicit user instruction).

## [2026-07-15 18:05] src-eurocommemo — Add backfill command for Sendcloud tracking on shipped eBay orders

**Target**: src-eurocommemo @ main
**Status**: SUCCESS
**Files affected**:
- `src/Command/BackfillSendcloudTrackingCommand.php` (new) — command `app:sendcloud:backfill-tracking`. Options `--orderId` (single order) and `--dry-run`. Iterates orders, resolves tracking read-only, sets `sendcloudParcelId`/`sendcloudTrackingNumber`/`sendcloudTrackingUrl`, flush by batches of 200 (skipped in dry-run), aborts on `ExternalSendcloudApiException`. Pattern mirrors `SyncSendcloudOrderIdsCommand`.
- `src/Service/Sendcloud/SendcloudTrackingResolver.php` (new, `final class`) — `resolveTracking(Order): ?array` queries `SendcloudApiClient::getShipmentsByOrderNumber(orderIdEbay)`, flattens `data[].parcels[]`, selects the latest parcel carrying a `tracking_number`, returns `{parcelId, trackingNumber, trackingUrl}` or null. No flush (read-only), never calls `createLabelSync`.
- `src/Service/Sendcloud/SendcloudApiClient.php` (modified) — added const `ENDPOINT_SHIPMENTS = '/shipments'` and `getShipmentsByOrderNumber(string $orderNumber): array` (GET `/api/v3/shipments?order_number=...`, `data` unwrap + `Link rel="next"` pagination, same pattern as `getOrders()`).
- `src/Repository/OrderRepository.php` (modified) — added `findEbayOrdersShippedMissingTracking(): Order[]` (`isEbay = true`, `sendcloudOrderId` NOT NULL and `!= '0'`, `dateExpedition` NOT NULL, orX on the three tracking columns being NULL).
- `tests/Service/Sendcloud/SendcloudTrackingResolverTest.php` (new) — 6 unit tests (Guzzle MockHandler), all green.

**Notes**: Implements `plans/2026-07-15_backfill-tracking-sendcloud.md`. Endpoint choice `/v3/shipments` confirmed against `specs/sendcloud-v3/shipments/openapi.yaml` (response `data[].parcels[]` with `id`/`tracking_number`/`tracking_url`, `order_number` query filter). Read-only backfill (no label regeneration). Verified in `php-fpm-per83` via `scripts/repo_exec.py`: command listed in `list app:sendcloud`; unit test 6/6 and full `tests/Service/Sendcloud` 15/15 green; query matches 3131 rows in prod-like DB; targeted `--dry-run` on 3 sample orders — 2 resolved real parcels/tracking (`CB551956536FR`, `XT331834572TS`), 1 legitimately skipped (no Sendcloud shipment for that order). **Not verified**: no full global run (3131 orders → 3131 API calls, left to the user); no live browser check that "Suivre le colis" then appears. No commit made (awaiting explicit user instruction).

## [2026-07-16 00:00] src-eurocommemo — EasyAdmin action to resolve Sendcloud tracking from the orders list

**Target**: src-eurocommemo @ main
**Status**: SUCCESS
**Files affected**:
- `src/Service/Sendcloud/UseCase/BackfillSendcloudTrackingUseCase.php` (new, `final class`) — `execute(Order): array` wraps `SendcloudTrackingResolver::resolveTracking()`, hydrates `sendcloudParcelId`/`sendcloudTrackingNumber`/`sendcloudTrackingUrl`, and on no parcel writes the sentinel `"0"` to all three columns (const `NO_PARCEL_SENTINEL`). Returns `{status: 'resolved'|'not_found', parcelId, trackingNumber, trackingUrl}`. Mutates the managed entity but does NOT flush (caller flushes) — shared reusable brick for the admin action and the batch command.
- `src/Controller/Admin/OrderCrudController.php` (modified) — injected `BackfillSendcloudTrackingUseCase`; added EasyAdmin action `resolveSendcloudTracking` ("Récupérer le suivi Sendcloud", icon `fa fa-barcode`) mirrored on `syncSendcloud` (`linkToUrl` + CSRF `sendcloud_tracking_<id>` query param, `displayIf`), registered on `Crud::PAGE_INDEX` + `reorder`; added CRUD method `resolveSendcloudTracking(AdminContext): RedirectResponse` (CSRF guard + business guard + `flush()` + `addFlash` success/warning/danger + referrer redirect, catches `ExternalSendcloudApiException`). `displayIf` shows the action only for eBay orders that are shipped (`dateExpedition` not null), linked to Sendcloud (`sendcloudOrderId` not null/`'0'`), and still missing at least one of the three tracking columns.
- `src/Command/BackfillSendcloudTrackingCommand.php` (modified) — refactored to delegate to the new UseCase instead of injecting `SendcloudTrackingResolver` directly; counters `updated`/`skipped` → `resolved`/`notFound` (no-parcel orders are now written to `"0"` instead of skipped); batch-flush every 200 and final flush preserved; summary line updated.
- `templates/admin/order/order_ebay_shipping_service.html.twig` (modified) — guarded the "Suivre le colis" link with `sendcloudTrackingUrl != '0'` so the sentinel value never renders a broken link.

**Notes**: Implements the approved plan `plans/2026-07-16_bouton-resync-tracking-sendcloud.md` (approach switched mid-plan from a custom AJAX route + Twig button + JS to a native EasyAdmin row action per user request). No JS/CSS/route/migration touched (the three Sendcloud columns already exist on `Order`). Verified in `php-fpm-per83` via `scripts/repo_exec.py`: `lint:container` OK (autowiring of the new UseCase + controller injection valid), `php -l` clean on all three PHP files, `lint:twig` OK on the template. **Not verified**: no live browser run against the admin list (action visibility, click → flash, DB hydration); no CLI dry-run re-run of the refactored command. No commit made (awaiting explicit user instruction).

## [2026-07-16 00:00] src-eurocommemo — Two filters on the "Toutes les commandes" admin view (shipping status + shipping service)

**Target**: src-eurocommemo @ main
**Status**: SUCCESS
**Files affected**:
- `src/Filter/OrderShippingStatusFilter.php` (new, `final class`) — EasyAdmin custom filter implementing `FilterInterface` + `FilterTrait`, backed by `ChoiceFilterType` (multiple), 5 choices matching the "Statut" column badges (En attente de paiement / Annulée / Expédiée / Prévente / En attente d'expédition). `apply()` reproduces the derived-status logic of `templates/admin/order/order_state.html.twig` in DQL: `statePayment` constants, `dateExpedition IS (NOT) NULL`, `(delivery IS NOT NULL OR isEbay)`, and a presale sub-query (`SELECT o2.id ... JOIN op2.product p2 WHERE p2.presale = true`) mirroring the `ready` block of `OrderCrudController::createIndexQueryBuilder`. Honours the IN / NOT IN comparison selector.
- `src/Controller/Admin/OrderCrudController.php` (modified) — added `use App\Filter\OrderShippingStatusFilter;` and `use EasyCorp\...\Filter\EntityFilter;`; in `configureFilters()`, gated on the "Toutes les commandes" view (`!$request->query->has('state')`), added `OrderShippingStatusFilter::new('shippingStatus', "Statut d'expédition")` and `EntityFilter::new('shippingService', 'Service de livraison')` with `value_type_options.choice_label => 'shippingService'` (ShippingService has no `__toString()`). No change to `createIndexQueryBuilder` (filters apply after it on the same `entity` alias); the pre-existing `leftJoin('entity.shippingService', ...)` is untouched (EntityFilter on a to-one uses `entity.shippingService IN (...)`, no extra join).

**Notes**: Implements the approved plan `plans/2026-07-16_filtres-statut-expedition-et-service-livraison-toutes-commandes.md`. No entity/migration/template change. Verified via `scripts/repo_exec.py` in the container: `php -l` clean on both PHP files; `cache:clear` OK (autoload PSR-4 `App\Filter\` + DI valid). **Not verified**: no live browser run (Filters panel rendering, service dropdown labels, per-status result correctness vs badges, non-regression on the `state=…` views). No commit made (awaiting explicit user instruction).

## [2026-07-16 17:52] src-eurocommemo — Async admin button to backfill Sendcloud tracking on shipped eBay orders

**Target**: src-eurocommemo (working tree, no branch/commit created)
**Status**: SUCCESS
**Files affected**:
- `src/Messenger/Message/BackfillSendcloudTrackingMessage.php` (new) — immutable DTO carrying a scalar `int $orderId` + `getOrderId()`, mirror of `AssociateSendcloudOrderIdMessage`.
- `src/Messenger/Handler/BackfillSendcloudTrackingHandler.php` (new, `#[AsMessageHandler]`) — reloads the `Order` via `OrderRepository::find()`, no-op if deleted, idempotency guard (returns early when the three tracking columns are already set — resolved or `'0'` sentinel), calls `BackfillSendcloudTrackingUseCase::execute()` then `flush()`. Does NOT catch `ExternalSendcloudApiException` so it propagates and triggers the `async_sendcloud` exponential retry (unlike `BackfillSendcloudTrackingCommand` which aborts the whole run).
- `config/packages/messenger.yaml` (modified) — routed `BackfillSendcloudTrackingMessage` to the existing `async_sendcloud` transport (retry: 10 attempts, exponential backoff capped at 5 min). Without this the message would run synchronously.
- `src/Repository/OrderRepository.php` (modified) — added `findEbayOrderIdsShippedMissingTracking(): int[]`, lightweight scalar-id variant of `findEbayOrdersShippedMissingTracking()` (same criteria via `getScalarResult()` + `array_map('intval', array_column(...))`) to fan-out messages without hydrating full entities.
- `src/Controller/Admin/OrderCrudController.php` (modified) — added imports (`BackfillSendcloudTrackingMessage`, `OrderRepository`, `MessageBusInterface`); injected `MessageBusInterface $messageBus` + `OrderRepository $orderRepository` in the constructor; in `configureActions()` added a global index action `backfillTrackingAll` ("Mettre à jour les suivis Sendcloud", `createAsGlobalAction()`, `btn btn-primary`, JS `confirm()`, CSRF token `sendcloud_tracking_all` in the URL, mirror of `resolveSendcloudTracking`) and registered it via `->add(Crud::PAGE_INDEX, ...)`; added the `backfillTrackingAll(AdminContext)` handler method — validates CSRF, loads ids, dispatches one `BackfillSendcloudTrackingMessage` per order, flashes an `info` "traitement en arrière-plan" message (or "aucune commande à mettre à jour" when empty).

**Notes**: Implements the approved plan `plans/2026-07-16_bouton-backfill-suivi-sendcloud-async.md`. No template change (flash already rendered by `admin/layout.html.twig`). Correction to the original brief noted in the plan: the backoffice single-shipment sync uses `BackfillSendcloudTrackingUseCase` (not `UpdateEbayOrderShippingServiceUseCase`); the async button reuses the former. Verified via `scripts/repo_exec.py` in the container: `php -l` clean on all 4 PHP files; `debug:messenger` lists the new message routed to its handler; `cache:clear` OK (DI container compiles → constructor autowiring valid). **Requires**: an active `messenger:consume async_sendcloud` worker (same one already consuming `AssociateSendcloudOrderIdMessage`) — messages otherwise queue in the `messenger_messages` table. **Not verified**: no live browser run (button rendering, click → dispatch → flash, worker draining the queue and populating tracking columns). No commit made (awaiting explicit user instruction).

## [2026-07-21 23:26] src-eurocommemo — Admin action to resend the shipping-tracking email on shipped orders

**Target**: src-eurocommemo @ `sendcloud-order-process` (working tree, no branch/commit created)
**Status**: SUCCESS
**Files affected**:
- `src/Service/MailService.php` (modified) — `sendMailExpeditionOrder(Order): void` becomes `: bool`. The `filter_var` guard is inverted into an early `return false`, the `catch (Exception)` returns `false` after the existing `critical` log, and the method returns `true` once the message is handed to the transport. The misleading `@throws TransportExceptionInterface` annotation is replaced by a `@return` docblock (the `catch` already swallowed it). Sole pre-existing caller `OrderListener.php:57` ignores the return value — automatic flow unchanged.
- `src/Controller/Admin/OrderCrudController.php` (modified) — added `use App\Service\MailService;` and `private readonly MailService $mailService` to the constructor; declared the per-row index action `sendShippingMail` ("Envoyer le mail de suivi", `fa fa-envelope`, `linkToUrl` + CSRF token `shipping_mail_<id>` in the query string, `confirm()` via `setHtmlAttributes`, `displayIf` on the shipped-web-order predicate), registered it in `->add(Crud::PAGE_INDEX, ...)` and in the `reorder`; added the `sendShippingMail(AdminContext): RedirectResponse` handler — CSRF guard, server-side re-check of the `displayIf` predicate (anti-forged URL), call to `MailService`, `success`/`danger` flash and a `warning` log on failure. No `flush()` (no entity mutation, no persisted send trace — explicit scoping decision).
- `templates/admin/order/order_state.html.twig` (modified) — the two "shipped/not shipped" branches now read `(delivery is not null or sendcloudServicePointId is not null or isEbay)`.
- `src/Filter/OrderShippingStatusFilter.php` (modified) — same extension applied to the three DQL predicates (`STATUS_SHIPPED`, `STATUS_PRESALE`, `STATUS_WAITING_SHIPMENT`).

**Notes**: Implements the approved plan `plans/2026-07-21_mail-suivi-expedition-commandes-expediees.md`. Scope: web orders only (`isEbay` not true) — eBay buyer emails are usually masked (`ImportBuyerDTO::maskedToNull`). No entity change, no migration, no asset rebuild.

**Deviation from the plan (user-approved mid-implementation)**: the plan's predicate `delivery !== null` excluded web **service-point** orders, because `CartHelper` never calls `setDelivery()` on the `sendcloudDelivery` branch (`CartHelper.php:271-286`) — those orders showed no status badge at all and would never have exposed the action, although `OrderListener` does set their `dateExpedition` and they are precisely the ones carrying a Sendcloud tracking URL. The predicate was therefore widened to `(delivery !== null || sendcloudServicePointId !== null)` in the action, in the badge template and in the status filter, keeping the three definitions in sync.

**Risk closed since the plan was written**: the Twig syntax error at `templates/mail/mail_delivery.html.twig:19` documented in the plan has been fixed in the working tree (unrelated WIP edit, not by this session) — `lint:twig` on that file is now clean.

Verified via `scripts/repo_exec.py` in the container: `php -l` clean on the three PHP files; `lint:container` OK (autowiring of `MailService` into `OrderCrudController`); `cache:clear` OK; `lint:twig` clean on `order_state.html.twig`; the widened filter predicate executed as DQL through `doctrine:query:dql` (3891 shipped web orders eligible for the action, 0 service-point ones so far). **Pre-existing, untouched**: `templates/order/pdf-invoice.html.twig:140` uses `||` instead of `or` and fails `lint:twig` (uncommitted WIP, unrelated); `OrderListener.php:52` mixes `&&`/`||` without parentheses, so any update of an order carrying a `sendcloudServicePointId` resets `dateExpedition` and re-sends the shipping email. **Not verified**: no live browser run (action rendering, click → confirm → flash, actual delivery of the email). No commit made (awaiting explicit user instruction).

## [2026-07-22 00:50] src-eurocommemo — Debug email of the raw Sendcloud webhook payload

**Target**: src-eurocommemo @ `main` (working tree, no branch/commit created)
**Status**: SUCCESS
**Files affected**:
- `.env` (modified) — added `SENDCLOUD_WEBHOOK_DEBUG_MAIL=1` in the `### Sendcloud ###` block, with a comment stating it is debug-only and how to stop it. Declared here so Symfony Dotenv provides a default and the container never fails to boot if the real environment omits the key.
- `config/services.yaml` (modified) — new parameter `sendcloud_webhook_debug_mail: "%env(bool:SENDCLOUD_WEBHOOK_DEBUG_MAIL)%"` next to `mailer_from` / `absolute_url`; the `bool:` cast makes `0` actually disable the feature.
- `templates/mail/sendcloud_webhook_debug.html.twig` (new) — standalone HTML fragment (same style as `sendcloud_link_failed.html.twig`, no `extends`): reception date, `action`, headers table (`values|join(', ')` since `Request::headers->all()` returns lists), query table with an `(vide)` fallback, and the body in a `<pre>` with `(vide)` fallback.
- `src/Service/MailService.php` (modified) — new `sendSendcloudWebhookDebug(string $action, array $headers, array $query, string $rawBody, ?array $decodedBody): void` inserted after `sendSendcloudLinkFailure()`. Mirrors the internal-alert pattern: hardcoded recipient `morvan.aurelien@gmail.com`, `from` built from `%mailer_from%`, `try/catch` around `send()` logging `critical`. Body is re-indented with `json_encode(..., JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE)` when the payload is JSON, raw otherwise. Does not touch `requestStack` (no `seo` request attribute on the webhook route).
- `src/Controller/WebhookController.php` (modified) — `webhookSendcloud()` now also takes `MailService $mailService` and `#[Autowire(param: 'sendcloud_webhook_debug_mail')] bool $debugMailEnabled` as action arguments (added `use App\Service\MailService;` and `use Symfony\Component\DependencyInjection\Attribute\Autowire;`). Headers/query/body are captured once into locals, the pre-existing `logger->warning()` is kept unchanged, and the mail is sent only when the flag is on. Non-JSON bodies decode to `null` and yield the `unknown` action. The `200` response is unconditional.

**Notes**: Implements the approved plan `plans/2026-07-22_sendcloud-webhook-debug-mail.md`. Purpose is temporary observation — cataloguing which `action` values and payload shapes Sendcloud actually emits — hence the env kill-switch rather than a code change to stop it. Design decisions taken with the user: synchronous send in the controller (no Messenger worker to guarantee for a debug device), env kill-switch, formatted HTML content.

**Discovered during verification (pre-existing, not introduced here)**: the Sendcloud webhook route is locale-prefixed — `debug:router webhook_sendcloud` resolves to `/{_locale}/sendcloud/notification` (`_locale: fr|en|de`, default `fr`), not `/sendcloud/notification`. The URL that must be registered in the Sendcloud panel is therefore `https://<host>/fr/sendcloud/notification`. `POST /sendcloud/notification` (no locale) does **not** match this route. Behaviour unchanged by this session, but it is worth confirming what is configured on the Sendcloud side, otherwise no notification ever reaches this controller.

**Security note**: headers are mailed verbatim, including `sendcloud-signature` and any cookie. Acceptable here (single recipient = system owner, temporary device); remember it when turning the flag off.

Verified via `scripts/repo_exec.py` in the container: `php -l` clean on both PHP files; `lint:twig` clean on the new template; `lint:container` OK; `cache:clear` OK; `debug:container --parameter=sendcloud_webhook_debug_mail` resolves. End-to-end against the running stack (`curl --resolve dev.eurocommemorative.docker:443:127.0.0.1`): (1) JSON `parcel_status_changed` payload with a `?foo=bar` query and a `Sendcloud-Signature` header → HTTP 200 and a mail `[Sendcloud][Webhook] parcel_status_changed` in Mailcatcher, headers/query tables and pretty-printed body all correct; (2) `text/plain` body `not-json` → HTTP 200 and a mail `[Sendcloud][Webhook] unknown` with the raw body and `(vide)` query, no 500; (3) flag temporarily set to `0` + `cache:clear` → HTTP 200, log line still written, no new mail (mailbox stayed at 2). The flag was restored to `1` afterwards. No commit made (awaiting explicit user instruction).

## [2026-07-25 16:05] src-eurocommemo — Fix: relay (Shop2Shop) web orders flagged "Expédiée" before their label exists

**Target**: src-eurocommemo @ `main` (working tree, no branch/commit created)
**Status**: SUCCESS
**Files affected**:
- `src/EventListener/OrderListener.php` (modified) — `onFlush()`: the "num suivi edited" guard was `array_key_exists('numSuiviLivraison', $changeSet) && !is_null($entity->getDelivery()) || !is_null($entity->getSendcloudServicePointId())`. PHP binds `&&` tighter than `||`, so the second operand alone was enough: **any** flushed update on an order carrying a `sendcloudServicePointId` dated it as shipped and mailed the customer. Rewritten as `array_key_exists(...) && !is_null(getNumSuiviLivraison()) && '' !== getNumSuiviLivraison() && (!is_null(getDelivery()) || !is_null(getSendcloudServicePointId()))`, and `setDateExpedition()` is now wrapped in `is_null(getDateExpedition())` so the timestamp already written by `SendcloudLabelService::generateLabel()` is preserved instead of overwritten. The `$unitOfWork->getEntityChangeSet($entity)` call, previously repeated three times in the loop, is hoisted into `$changeSet` and reused by the two `statePayment` blocks (same value, no semantic change).
- `tests/EventListener/OrderListenerTest.php` (new) — 7 plain PHPUnit unit tests (mocked `EntityManagerInterface` + `UnitOfWork`, style of `tests/Messenger/Handler/AssociateSendcloudOrderIdHandlerTest.php`): relay order not shipped when created on Sendcloud, relay order not shipped when the payment IPN validates it (the reported bug), relay order shipped when a tracking number is recorded, existing shipping date preserved, tracking cleared to `''` is not a shipment, classic web order with a `delivery` still shipped, eBay order still receives no customer mail.

**Notes**: Implements the plan approved in session (`~/.claude/plans/peux-tu-me-dire-comment-resilient-reef.md`). Scope explicitly limited by the user to the listener.

Impact beyond the wrong badge: the whole Sendcloud admin panel is wrapped in `dateExpedition is null` (`templates/admin/order/order_sendcloud_action.html.twig`) and the routes `sendcloudApplyShipping` / `sendcloudGenerateLabel` (`OrderCrudController.php:778`, `:817`) reject a dated order — so a relay order used to lose the ability to pick a shipping method or generate its label for good, right after payment. Three flush paths reached the faulty guard: `PaymentController::donePaymentCBAction()` (IPN), `SendcloudOrderCreator::createForOrder()` (dispatched by `CreateSendcloudOrderHandler`), `OrderCrudController::sendcloudApplyShipping()`.

**No data repair needed**: the 5 relay orders in base (ids 40375-40380, `sendcloud_service_point_id NOT NULL AND is_ebay IS NULL`) are all `state_payment = 0`, so none had been paid yet and none carries a wrong `date_expedition`.

**Still open (not in scope, pre-existing)**: once a relay order is paid, `templates/admin/order/order_delivery_action.html.twig` matches its first branch (`delivery is null and collected == false and not isEbay`) and offers "Confirmer la récupération de la commande", which routes to `admin_delivery_validate()` → `setCollected(true)` and removes the order from the "to prepare" list (`OrderCrudController.php:140`). A relay order is not a click-and-collect order; both the template branch and the controller branch should exclude orders carrying a `sendcloudServicePointId`. The "Expédition" column also stays empty for a relay order (its branches only cover `delivery is not null` / `isEbay`). *(Fixed in the next entry.)*

Verified via `scripts/repo_exec.py` in the container: `php -l` clean on `OrderListener.php`; new suite green (7 tests, 14 assertions); full suite green (26 tests, 48 assertions, no regression on the 4 pre-existing suites). Regression value proven by temporarily restoring the old condition — 3 of the 7 tests fail (relay order created on Sendcloud, relay order paid, tracking cleared), then pass again once the fix is restored. SQL control on `eurocosv2` returns 0 rows: `SELECT COUNT(*) FROM orders WHERE is_ebay IS NULL AND sendcloud_service_point_id IS NOT NULL AND date_expedition IS NOT NULL AND num_suivi_livraison IS NULL`. **Not verified**: no live browser run of the admin flow (pay a relay order → badge "En attente d'expédition" → Sendcloud panel visible → generate label → badge "Expédiée" + mail), no relay order being payable in the current data set. No commit made (awaiting explicit user instruction).

## [2026-07-25 16:09] src-eurocommemo — Fix: "Confirmer la récupération" offered on relay orders

**Target**: src-eurocommemo @ `main` (working tree, no branch/commit created)
**Status**: SUCCESS
**Files affected**:
- `templates/admin/order/order_delivery_action.html.twig` (modified) — a relay order has no `delivery` either, so it used to match the click-and-collect branch and display "Confirmer la récupération de la commande". Both collect branches (`collected == false` and `collected == true`) now also require `sendcloudServicePointId is null`. The shipped line was widened from `delivery is not null` to `(delivery is not null or sendcloudServicePointId is not null)` so the "Expédition" column still reads "Commande expédiée le …" for a relay order instead of going blank — the branch it used to fall into was the one just removed. The tracking-input branch (`(delivery is not null or isEbay) and dateExpedition is null`) is deliberately left as is: a relay order is shipped from the "Sendcloud" column (label generation), not by typing a tracking number by hand.
- `src/Controller/Admin/OrderCrudController.php` (modified) — `admin_delivery_validate()`: new guard before the collect branch. An order with no `delivery`, not eBay and carrying a `sendcloudServicePointId` returns `['state' => 0, 'message' => "Cette commande est expédiée en point relais : générez l'étiquette Sendcloud."]` instead of falling into `setCollected(true)`. Defence in depth — the button no longer renders, but the route stays reachable.

**Notes**: Follow-up explicitly requested by the user on the item left open by the previous entry. Same session, same subject (relay orders wrongly treated as shipped/collected), but a separate change: it belongs in its own commit.

**Rendering matrix for a paid relay order** after the fix: not yet shipped → column empty (the action lives in the "Sendcloud" column next to it); shipped → "Commande expédiée le …". Adding an explicit "en attente de l'étiquette" hint was left out on purpose (not requested).

**Zero impact on existing data**, checked in SQL by category on paid web orders (`is_ebay IS NULL AND state_payment = 1`): 0 relay orders (all 5 are still unpaid), so no row changes rendering today. The 5 real click-and-collect orders (`delivery_id NULL`, `sendcloud_service_point_id NULL`, `collected = 1`) and the 3 890 orders with a delivery method keep the exact same branches.

**Discovered, pre-existing and untouched**: 19 538 paid web orders have `delivery_id NULL`, `collected = 0` and a `date_expedition` (legacy `SyncRefonteCommand` imports, which never set `delivery_id`). They match the click-and-collect branch and are therefore offered "Confirmer la récupération de la commande" although they were shipped years ago. Behaviour identical before and after this change; fixing it would mean adding `dateExpedition is null` to that branch.

Verified via `scripts/repo_exec.py` in the container: `php -l` clean on `OrderCrudController.php`; `lint:twig` OK on `order_delivery_action.html.twig`; `lint:container` OK; full suite still green (26 tests, 48 assertions). **Not verified**: no live browser run of the admin list (button rendering per category, click → error message on a relay order) — no relay order is payable in the current data set. No commit made (awaiting explicit user instruction).

## [2026-07-25 17:27] src-eurocommemo — Fix: Shop2Shop logo missing on the PDF invoice (web and eBay relay orders)

**Target**: src-eurocommemo @ `main` (working tree, no branch/commit created)
**Status**: SUCCESS
**Files affected**:
- `templates/order/pdf-invoice.html.twig` (modified) — line 142, single token. Commit `7370656` ("fix(order): improve visibility settings for shipping logos", 2026-07-22) had written the guard as `shippingService and shippingService.showOnInvoice and showOnInvoice.logoName`. `showOnInvoice` is not a template variable — both render sites (`OrderHelper.php:138`, `GenerateAndConcatePdfInvoiceUseCase.php:35`) pass only `order`. With `strict_variables` enabled solely in the `test` environment (`config/packages/twig.yaml:16`), Twig silently resolved `showOnInvoice.logoName` to `null`, so the condition was **always false and the `<img>` never rendered**, for every order origin. Corrected to `shippingService.logoName`. The now-redundant outer guard on line 140 was deliberately left untouched.
- `src/Twig/GetShippingServiceFromOrderExtension.php` (modified) — the fallback used by web relay orders had `"FR_Shop2Shop"` hardcoded since `e9bea50`, so **any** Sendcloud option code resolved to the Shop2Shop service. Replaced by an explicit `SENDCLOUD_CODE_TO_EBAY_TOKEN` map (`'chronopost:shop2shop' => 'FR_Shop2Shop'`), and `findOneBy(['shippingService' => …])` swapped for the existing `ShippingServiceRepository::findOneByToken()` helper. An unmapped code now returns `null` (no logo) instead of a wrong logo.

**Notes**: Implements the plan approved in session (`~/.claude/plans/sur-la-g-n-ration-de-jazzy-hamming.md`). Test scope explicitly set to none by the user; hardening of the Sendcloud mapping explicitly requested.

**Rest of the chain verified sound, nothing else to change**: `ebay_shipping_service` id 116 has `shipping_service = FR_Shop2Shop`, `show_on_invoice = 1`, `logo_name = 6a4593e195cf5371592026.png` (the only row with `show_on_invoice = 1`); the file exists in `public/uploads/images/logo/`; the `{{ absolute_url }}/uploads/images/logo/…` path mirrors the header logos on lines 13 and 23, which do print.

**Zero impact on existing data** from the mapping change: the 6 relay orders in base all carry `chronopost:shop2shop` — order 40374 (eBay, `shipping_service_id = 116`, `state_payment = 1`) and orders 40375-40380 (web, `shipping_service_id` NULL, unpaid).

Verified via `scripts/repo_exec.py` in the container: `php -l` clean; `lint:twig` OK on `pdf-invoice.html.twig`; `lint:container` OK; `cache:clear` OK; full suite still green (26 tests, 48 assertions). End-to-end rendering checked with a throwaway console command (`scratch:verify-invoice-logo`, **deleted afterwards, not committed**) that rendered the real template against the real database with `enableStrictVariables()` forced on: order 40374 (eBay branch) and order 40375 (web fallback branch) both resolve `FR_Shop2Shop` and both emit the `<img>`; an in-memory order carrying `mondialrelay:relais` resolves to `null` (negative control for the hardened map). Regression value proven by temporarily restoring the faulty condition — the same run then aborts with `Variable "showOnInvoice" does not exist.` at `pdf-invoice.html.twig line 142`, which in dev/prod degrades silently into a missing logo. **Not verified**: no browser run of the actual PDF download (`_order_invoice_download_pdf` on order 40374 requires an authenticated admin session), so the Dompdf remote fetch of the logo over `ABSOLUTE_URL` was not exercised end to end. No commit made (awaiting explicit user instruction).

**Note for the future**: this class of bug is invisible in dev and prod because `strict_variables` is only on in `test`. Any silent "block that no longer renders" in a Twig template is worth checking with `enableStrictVariables()` first.

## [2026-07-25 18:05] src-eurocommemo — Restrict the Sendcloud webhook debug email to real parcel notifications

**Target**: src-eurocommemo @ `main` (working tree, no branch/commit created)
**Status**: SUCCESS
**Files affected**:
- `src/Controller/WebhookController.php` (modified) — `webhookSendcloud()`. The debug email introduced by `1421a35` fired on every Sendcloud POST. It is now sent only when both conditions hold: (1) the decoded body carries a `parcel` key holding an array; (2) `parcel.email` does not contain `@members.ebay.com`. Either check failing returns `200` with no mail, so Sendcloud never sees a failed delivery and stops retrying. Added the private constant `EBAY_MEMBERS_EMAIL_DOMAIN`; the comparison lowercases the address since a mail domain is case-insensitive.
